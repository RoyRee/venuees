...
import { api, type VenueListItem, type Getaway, type Destination, type RealWedding } from "../lib/api";
import { venueHero, getawayPhotos, destinationPhotos, realWeddingPhotos, signatureResortsPhotos } from "../lib/images";
import { HeroSearch } from "../components/hero-search";

const SITE = { name: "Venuees.in", established: 2018 };

export default function HomePage() {
  const { data: venues = [] } = useQuery({
    queryKey: ["venues"],
    queryFn: () => api.venues.list({ city: "nagpur" }),
  });
  const { data: getaways = [] } = useQuery({
    queryKey: ["getaways"],
  ...
  const { data: realWeddings = [] } = useQuery({
    ...
    <div className="home">
      ...
      <section className="hero">
        <div className="hero-bg">
          <Photo src={signatureResortsPhotos.hero} variant="garden" label="signature resorts · wardha road" style={{ height: "100%" }} />
        ...
        <div className="container">
          <div className="hero-trust">
            <div><span className="num">{venues.length || 42}</span><span>Handpicked venues</span></div>
          ...
          <div className="sig-inner">
            ...
            <div>
              ...
              <p style={{ fontSize: 16, lineHeight: 1.7, color: "var(--ink-soft)", marginBottom: 22 }}>
                Our flagship — 14 landscaped acres on Wardha Road, ten minutes from the airport. Four venues inside one compound. Sixty-eight guest rooms on-site, so the baraat, the bride's family, and the band all stay under one roof.
              </p>
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap", marginBottom: 24, fontSize: 13, color: "var(--ink-soft)" }}>
                ...
                {signature.rooms && <span><I.Bed width={14} height={14} /> {signature.rooms} rooms on-site</span>}
                <span><I.Car width={14} height={14} /> {signature.parking} cars · valet</span>
                <span><Stars value={Number(signature.rating)} size={13} /> {signature.rating} · {signature.reviews} reviews</span>
              </div>
      ...
      <section className="block">
        <div className="block-head">
          ...
          <Link href="/venues" className="block-link">All {venues.length || 42} venues <I.Arrow width={12} height={12} /></Link>
        </div>
        <div className="vgrid">
          {(featured.length ? featured : Array.from({ length: 4 }) as VenueListItem[]).map((v, i) =>
            v ? (
              <Link key={v.slug} href={`/venues/nagpur/${v.locality.split(",")[0].toLowerCase().replace(/\s+/g, "-")}/${v.slug}`} className="vcard">
                <Photo src={v.primaryImage ?? venueHero(v.slug)} variant={v.ph as "v2"} label={v.name.toLowerCase()} className="vcard-img" style={{ height: 220 }}>
                  <div className="badges-top"><span className="badge-assured">{v.tag}</span></div>
              ...
              <div key={i} className="vcard" style={{ background: "var(--surface)", height: 340, borderRadius: "var(--radius-md)", opacity: 0.4 }} />
            )
          )}
        </div>
      </section>
          ...
          <div className="getaway-block">
            <div className="getaway-intro">
              ...
              <p style={{ fontSize: 15, lineHeight: 1.7, color: "var(--ink-soft)", marginTop: 16 }}>
                Pench, Tadoba, Khindsi, Chikhaldara — forest villas and lake cottages that turn a Friday evening into a proper reset.
              </p>
            ...
            <div className="gcards">
              {getaways.slice(0, 3).map((g) => (
                <Link key={g.slug} href={`/weekend-getaways/${g.slug}`} className="gcard">
                  <Photo src={g.images[0]?.url ?? getawayPhotos[g.slug]?.hero} variant={g.ph as "v2"} label={g.name.toLowerCase()} style={{ height: 180 }} />
                  <div className="gcard-body">
                    ...
                    <div className="gcard-price">
                      <div><b>₹{g.weekday.toLocaleString("en-IN")}</b><small>/night · wknight</small></div>
            ...
            <div className="destgrid">
              {destinations.slice(0, 4).map((d) => (
                <Link key={d.slug} href={`/destination-weddings/${d.slug}`} className="destcard">
                  <Photo src={destinationPhotos[d.slug]} variant={d.ph as "v2"} label={`${d.city.toLowerCase()} · ${d.tag.toLowerCase()}`} />
                  <div className="destcard-lbl">
                    <div>
                      <div style={{ fontFamily: "var(--font-serif)", fontSize: 28, color: "#FFF8EA", letterSpacing: "-0.02em" }}>{d.city}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,248,234,0.8)", marginTop: 3 }}>{d.venues} venues · from {d.priceFrom}</div>
                    </div>
      ...
      <section className="block">
        ...
        <div className="steps">
          <div className="step">
            ...
            <p>Share your dates, guest count, and budget. We&rsquo;ll pull real availability across {venues.length || 42} Nagpur venues in under an hour.</p>
          </div>
          <div className="step">
          ...
          <div className="realgrid">
            {realWeddings.slice(0, 5).map((w, i) => (
              <Link key={w.slug} href={`/real-weddings/${w.slug}`} className={`realcard r${i}`}>
                <Photo src={w.images[0]?.url ?? realWeddingPhotos[w.slug]} variant={w.ph as "v2"} label={w.couple.toLowerCase()} style={{ height: "100%" }} />
      ...
      {realWeddings.length > 0 && realWeddings[0]?.quote && (
        <section className="testimonial">
          <Ornament>A COUPLE&apos;S NOTE</Ornament>
          <blockquote>
            &ldquo;{realWeddings[0].quote}&rdquo;
          </blockquote>
          <div className="testi-by">
            <div className={`testi-avatar ph ${realWeddings[0].ph}`} />
            <div style={{ fontSize: 13 }}>
              <div style={{ fontWeight: 600 }}>{realWeddings[0].couple}</div>
              <div style={{ color: "var(--ink-mute)" }}>{realWeddings[0].venue} · {realWeddings[0].date} · {realWeddings[0].guests} guests</div>
            </div>
          </div>
        </section>
      )}
