...
function Field({ icon, label, children, last = false }: { icon: React.ReactNode; label: string; children: React.ReactNode; last?: boolean }) {
  return (
...
function SelectField({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: { label: string; value: string }[] }) {
  return (
    <select className="hs-select" value={value} onChange={(e) => onChange(e.target.value)}>
      {options.map((o) => (
  ...
  return (
    <div className="hs">
      <div className="hs-tabs" ref={tabBarRef} role="tablist">
        {TABS.map((t) => (
          <button key={t.id} role="tab" aria-selected={tab === t.id} className={`hs-tab${tab === t.id ? " active" : ""}`} onClick={() => setTab(t.id)}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="hs-fields" role="tabpanel">
        {tab === "venues" && (
          <>
            ...
            <Field icon={<I.Cal width={17} height={17} />} label="WHEN">
              <SelectField value={venueDate} onChange={setVenueDate} options={MONTHS.map((m) => ({ label: m, value: m }))} />
            </Field>
            <Field icon={<I.Users width={17} height={17} />} label="GUESTS">
              <SelectField value={venueGuests} onChange={setVenueGuests} options={GUEST_OPTIONS} />
            </Field>
          </>
        )}
        ...
        {tab === "getaways" && (
          <>
            ...
            <Field icon={<I.Cal width={17} height={17} />} label="WHEN">
              <SelectField value={getawayDate} onChange={setGetawayDate} options={MONTHS.map((m) => ({ label: m, value: m }))} />
            </Field>
          </>
        )}
        ...
        {tab === "destinations" && (
          <>
            <Field icon={<I.Pin width={17} height={17} />} label="DESTINATION">
              <SelectField value={destination} onChange={setDestination} options={DESTINATIONS} />
            </Field>
            <Field icon={<I.Users width={17} height={17} />} label="GUESTS">
              <SelectField value={destGuests} onChange={setDestGuests} options={GUEST_OPTIONS} />
            </Field>
          </>
        )}
        ...
        {tab === "vendors" && (
          <>
            <Field icon={<I.Search width={17} height={17} />} label="SERVICE">
              <SelectField value={vendorCategory} onChange={setVendorCategory} options={VENDOR_CATEGORIES} />
            </Field>
            <Field icon={<I.Pin width={17} height={17} />} label="WHERE">
              <span className="hs-fixed">Nagpur</span>
            </Field>
